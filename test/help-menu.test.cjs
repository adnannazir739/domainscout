...
test('native Help menu contains Store-ready support, update, and legal links', () => {
  for (const label of ['Help & Support', 'Check for Updates', 'Visit Domain Scout Website', 'Contact Us', 'Privacy Policy', 'Terms of Use', 'Affiliate Disclosure', 'About Domain Scout AI']) {
...
test('public website links use the verified HTTPS domain', () => {
  ...
  assert.match(main, /https:\/\/domainscout\.vortixvpn\.com\/version\.json/);
});
